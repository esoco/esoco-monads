...
The `Call` monad is quite similar to `Try`. But instead of executing the wrapped code only once, a call will execute it every time it is queried with the consuming/terminating methods. Hence, an invocation of `orUse`, `orGet`, `orFail`, and `orThrow` will cause a re-execution of the supplier, yielding a new result if the supplier is implemented accordingly. Calls can be arbitrarily mapped (including `then` and `orElse`) without being executed. Only consuming a call will invoke the wrapped code or perform error handling. This allows to store arbitrary function calls including any value transformations and error handling in variables and execute them on demand. 

Because the main purpose of this monad is to provide repeatable function executions it also provides the method `execute()`, which executes the call once and triggers any mappings or throws a `RuntimeException` on errors if no error handler has been defined before with `orElse(Consumer<Exception>)`. `execute(Consumer<Exception>))` also executes the call and forwards errors to an explicit error handler.

A new call instance can be created with `Call.of(Supplier)`. `Call.error(Exception)` creates an instance that always fails. The methods `toTry()` and `toLazy()` convert a call to a corresponding, single-execution `Try`.

`Call` is also a good example for the composition of monads, as the handling of the call executions is delegated to the `Try` monad. As another example, lazy tries use an `Option<Try<T>>` internally to model the delayed execution.